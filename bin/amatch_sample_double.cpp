...
{
    if(argc < 3) {
        printf("Usage: amatch_sample_double <track filename> <sample filename> \n\n");
        return 1;
    }
	...
	printf("amatch_sample_double: version 1.1\n\n"); 
    ...
    match_single_sample_double_pass(track_keys, sample_keys, 
           start_sec_of_track, end_sec_of_track, 
            0, sample_size_secs, 
            0, nsecs_to_match);
 
    double end_t = time_now();
    printf("match_single_sample_double_pass() Elapsed: %f\n", end_t - start_t);
